...
infer_example_metrics_path = os.path.join(efficiency_log_dir, 'infer_example_metrics.jsonl')
infer_batch_metrics_path = os.path.join(efficiency_log_dir, 'infer_batch_metrics.jsonl')
infer_summary_path = os.path.join(efficiency_log_dir, 'inference_stats.json')
run_metadata_path = os.path.join(efficiency_log_dir, 'run_metadata_eval.json')
...
num_ref_params = utils.count_parameters(ref_model, trainable_only=False)
num_value_params = utils.count_parameters(classifier_model, trainable_only=False)
if save_raw_efficiency:
...
for i in range(num_samples):
    ...
    for j in tqdm(range(num_batches)):
        ...
        utils.sync_cuda(sync_cuda_timing)
        tokenize_wall_sec = time.time() - tokenize_start
        generate_kwargs['output_scores'] = True
        generate_kwargs['return_dict_in_generate'] = True
        logit_processor.total_classifier_calls = 0
        logit_processor.total_classifier_batch_size = 0
        utils.sync_cuda(sync_cuda_timing)
        ...
        utils.sync_cuda(sync_cuda_timing)
        generate_wall_sec = time.time() - generate_start
        ...
        utils.sync_cuda(sync_cuda_timing)
        kl_wall_sec = time.time() - kl_start
        del aligned_model_scores
        torch.cuda.empty_cache()
        utils.sync_cuda(sync_cuda_timing)
        batch_total_wall_sec = time.time() - batch_total_start
        batch_size_current = len(batch_indices)
        output_attention_mask = (current_outputs_id != tokenizer.pad_token_id).long()
        generated_tokens_per_example = output_attention_mask.sum(dim=1).cpu().tolist()
        prompt_tokens_per_example = current_inputs['attention_mask'].sum(dim=1).cpu().tolist()
        guidance_calls = int(logit_processor.total_classifier_calls)
        guidance_call_to_example_ratio = guidance_calls / max(batch_size_current, 1)
        ...
        for k in range(len(batch_indices)):
            ...
            if save_raw_efficiency:
                generated_tokens = int(generated_tokens_per_example[k])
                prompt_tokens = int(prompt_tokens_per_example[k])
                ref_only_flops_est = float(2.0 * num_ref_params * generated_tokens)
                guided_flops_est = float(2.0 * (num_ref_params + num_value_params * guidance_call_to_example_ratio) * generated_tokens)
                utils.append_jsonl(infer_example_metrics_path, {
                    'repeat_index': int(repeat_index),
                    'batch_index': int(j),
                    'eval_problem_id': int(eval_problem_id),
                    'prompt_tokens': prompt_tokens,
                    'generated_tokens': generated_tokens,
                    'traj_kl': float(traj_kl[k].item()),
                    'tokenize_wall_sec_per_example': float(tokenize_wall_sec / max(batch_size_current, 1)),
                    'generate_wall_sec_per_example': float(generate_wall_sec / max(batch_size_current, 1)),
                    'kl_wall_sec_per_example': float(kl_wall_sec / max(batch_size_current, 1)),
                    'total_wall_sec_per_example': float(batch_total_wall_sec / max(batch_size_current, 1)),
                    'guidance_calls_batch': guidance_calls,
                    'guidance_calls_per_example': float(guidance_call_to_example_ratio),
                    'ref_only_flops_est': ref_only_flops_est,
                    'guided_flops_est': guided_flops_est,
        ...
        if save_raw_efficiency:
            ...
            utils.append_jsonl(infer_batch_metrics_path, {
                'repeat_index': int(repeat_index),
                'batch_index': int(j),
                'num_examples': int(batch_size_current),
                'prompt_tokens_total': int(sum(prompt_tokens_per_example)),
                'generated_tokens_total': int(sum(generated_tokens_per_example)),
                'tokenize_wall_sec': float(tokenize_wall_sec),
                'generate_wall_sec': float(generate_wall_sec),
                'kl_wall_sec': float(kl_wall_sec),
...
if save_raw_efficiency:
    inference_wall_time_sec = 0.0
    total_examples = 0
    total_generated_tokens = 0
    total_prompt_tokens = 0
    total_ref_only_flops = 0.0
    total_guided_flops = 0.0
    if os.path.exists(infer_example_metrics_path):
        for row in read_jsonl(infer_example_metrics_path):
            inference_wall_time_sec += row['total_wall_sec_per_example']
            total_examples += 1
            total_generated_tokens += row['generated_tokens']
    ...
    with open(infer_summary_path, 'w') as f:
        json.dump({
            ...
            'inference_wall_time_sec': float(inference_wall_time_sec),
            'avg_example_time_sec': float(inference_wall_time_sec / max(total_examples, 1)),
            'avg_generated_tokens': float(total_generated_tokens / max(total_examples, 1)),
            'tokens_per_sec': float(total_generated_tokens / max(inference_wall_time_sec, 1e-12)),
            'total_ref_only_flops_est': float(total_ref_only_flops),
            'total_guided_flops_est': float(total_guided_flops),

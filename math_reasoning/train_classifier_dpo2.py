...
def eval_dpo(model, dataloader, ref_model, beta=0.1):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ref_logps = compute_log_probs(ref_model, batch)
            loss, _, _ = dpo_loss(model, batch, beta=beta, ref_logps=ref_logps)
            total_loss += loss.item()
            accuracy = compute_dpo_accuracy(model, batch, beta=beta, ref_logps=ref_logps)
...
for epoch in range(args.num_epochs):
    ...
    for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}"):
        with accelerator.accumulate(model):
            ...
            loss, logp_c, logp_r = dpo_loss(model, batch, beta=args.eta, ref_logps=ref_logps)
            ...
            del batch, ref_logps, loss, logp_c, logp_r
            torch.cuda.empty_cache()

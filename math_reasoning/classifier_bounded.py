    ...
    def forward(
        ...
        if self.classifier_type == "Q":
            if labels is not None:
                ...
                logits = self.head_forward(hidden_states)  # [bs, seqlen, vocab_size] or [bs, seqlen, vocab_size * num_atoms]
                if self.loss_type in ["mse", "bce"]:
                ...
                logits = self.head_forward(hidden_states)  # [bs, vocab_size * (1 or num_atoms or 2)]
            ...
            if labels is not None:
                ...
                logits = self.head_forward(hidden_states)
...
class CustomValueGuidedLogitProcessor(LogitsProcessor):
    ...
    def __init__(self, eta, ref_model, ref_model_tokenizer, value_classifier, inference_mode, top_k, cd_baseline=0, use_cache=True):
        ...
        self.total_classifier_calls = 0
        self.total_classifier_batch_size = 0
    ...
    def get_classifier_values(self, input_ids, top_k_indices):
        ...
        self.total_classifier_calls += 1
        self.total_classifier_batch_size += input_ids.size(0)
